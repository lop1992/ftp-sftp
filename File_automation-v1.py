...
class transfer(object):
    ...
    def __init__(self):
        ...
        self.DATE_NOW = datetime.now().date().isoformat()           #设置当天时间
        self.DATE_MORE = datetime.now().strftime('%Y-%m-%d %H:%M')  #设置详细时间
    ...
    def FTP_synchronization(self):
        ...
        self.SENSE_file(self.DATE_NOW + '_updata.txt')                                  #判断同步更新文件是否存在
        ...
        with open(self.LOG_PATH +  'ftp_server_file_list.txt','a+') as file_server_list:#a代表以读写追加的方式打开文件，with open...可以不用close文件
            for suml in self.ftp.nlst():                                                #把FTP服务器的文件遍历并添加到filelog文件下
                if suml in self.F_S_LIST:
                    ...
                    with open(self.LOG_PATH+(self.DATE_NOW)+'_updata.txt','a') as f_updata:
                        f_updata.write(suml+'\n')
    ...
    def FTP_download(self):
        ...
        self.SENSE_file(self.DATE_NOW + '_D_log.txt')
        self.SENSE_file(self.DATE_NOW + '_down.txt')                                    #判断文件是否存在
        self.UPDATE_list(self.LOG_PATH + self.DATE_NOW + '_updata.txt',self.F_U_LIST,'r')        
        self.UPDATE_list(self.LOG_PATH + self.DATE_NOW + '_down.txt',self.F_D_LIST,'r')
        ...
        for suml in self.F_U_LIST:
            if suml in self.F_D_LIST:
                ...
                print('%s 文件%s 下载成功\n'%(self.DATE_MORE,suml))
                with open(self.LOG_PATH + self.DATE_NOW + '_down.txt','a') as f_downup:
                    f_downup.write(suml+'\n')
                    
                with open(self.LOG_PATH + self.DATE_NOW + '_D_log.txt','a') as f_log:
                   print('%s <--*-->成功下载文件<--*-->  %s'%(self.DATE_MORE,suml),file=f_log)
        
    def SFTP_upload(self):
        '''完成sftp文件上传功能'''
        self.SENSE_file(self.DATE_NOW + '_C_log.txt')
        ...
        self.SENSE_file(self.DATE_NOW + '_commit.txt')
        self.UPDATE_list(self.LOG_PATH + self.DATE_NOW + '_commit.txt',self.F_C_LIST,'r')
        for suml in self.F_D_LIST:
            if suml in self.F_C_LIST:
                pass
                #print('文件已已经存在!')
            else:
                sftp.put(self.DOWNLOAD_FILE + suml,self.SFTP_REMOTE_FILE + suml)        #上传文件
                print('%s 文件%s 上传成功\n'%(self.DATE_MORE,suml))
                            
                with open(self.LOG_PATH + self.DATE_NOW + '_commit.txt','a') as f_commit:
                    f_commit.write(suml+'\n')
                                
                with open(self.LOG_PATH + self.DATE_NOW + '_C_log.txt','a') as f_log:
                   print('%s <--*-->成功上传文件<--*-->  %s'%(self.DATE_MORE,suml),file=f_log)
