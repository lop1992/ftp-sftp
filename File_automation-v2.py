...
class Myftp(transfer):
    ...
    def __init__(self,FTP_HOST,FTP_USER_NAME,FTP_PASSWORD):
...
class Mysftp(transfer):
    def __init__(self,SFTP_HOST,SFTP_PORT,SFTP_USER_NAME,SFTP_PASSWORD):
            
        self.sftp = ''
        self.SFTP_HOST = SFTP_HOST                            	    #主机
        self.SFTP_PORT = SFTP_PORT                                  #端口
        self.SFTP_PASSWORD = SFTP_PASSWORD                          #密码
        self.SFTP_USER_NAME = SFTP_USER_NAME                        #用户名
    ...
    def upload(self):
        ...
        for suml in self.F_D_LIST:
            if suml in self.F_C_LIST:
                ...
                self.sftp.put(self.DOWNLOAD_FILE + suml,(self.SFTP_REMOTE_FILE + suml).encode("gbk"))#上传文件
                print('%s 文件%s 上传成功\n'%(datetime.now().strftime('%Y-%m-%d %H:%M'),suml))
...
if __name__ == '__main__':
    ...
    ftp = Myftp('192.168.1.150','lzh','ts123456')
    sftp = Mysftp('192.168.1.150',22,'lzh','ts123456')
    ftp.connect()
    ftp.login()
    sftp.login()
